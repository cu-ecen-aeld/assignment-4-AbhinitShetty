...
* then obtains the mutex in 'mutex', 
* then holds for 'wait_to_release_ms' milliseconds, then releases.

* The 'start_thread_obtaining_mutex' function should only start the thread and should not block for the thread to complete.
* The 'start_thread_obtaining_mutex' function should use dynamic memory allocation for thread_data structure passed into the thread.  
* The number of threads active should be limited only by the amount of available memory.
...
* If a thread was started succesfully 'thread' should be filled with the 'pthread_create' thread ID coresponding to the thread which was started.
